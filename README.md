...
Dream is a typed programming language with familiar `fun` / `let` syntax. You write one program; it compiles to WebAssembly and can run on your computer, in the browser, or in Node. Memory is automatic reference counting (ARC) — no GC pauses, no manual `free`.
