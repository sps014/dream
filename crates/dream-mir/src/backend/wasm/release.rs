...
fn emit_release_prologue(f: &mut FuncBuilder) {
    f.local_get("ptr");
    f.i32_eqz();
    f.if_();
    f.return_();
...
pub(super) fn emit_release_funcs(
    ...
    for layout in mir.layouts.structs.values() {
        ...
        let mut f = FuncBuilder::new(format!("release_{}", layout.name));
        f.param("ptr", ValType::I32);
        f.local("rc", ValType::I32);
        f.local("nc", ValType::I32);
        f.local("__wbox", ValType::I32);
        emit_release_prologue(&mut f);
        emit_del_call(&mut f, del.as_deref());
        for field in layout
            .fields
            .iter()
            .filter(|field| interner.is_rc_tracked(field.ty) && !field.is_weak && !field.is_unowned)
        {
    ...
    for layout in mir.layouts.unions.values() {
        ...
        for v in &layout.variants {
            let value_fields: Vec<&dream_hir::FieldLayout> = v
                .fields
                .iter()
                .filter(|field| interner.is_value_type(field.ty) && value_glue.contains(&field.ty))
                .collect();
    ...
    for elem in array_elem_types(mir, interner) {
        ...
        let mut f = FuncBuilder::new(format!("release_array_t{}", elem.0));
        f.param("ptr", ValType::I32);
        f.local("rc", ValType::I32);
        f.local("nc", ValType::I32);
    ...
    let mut f = FuncBuilder::new("release_object");
    f.param("ptr", ValType::I32);
    ...
    for (ty, layout) in &mir.layouts.structs {
        if let Some(&tag) = tags.get(ty) {
            ...
            f.if_();
            f.local_get("ptr");
            f.call(&format!("release_{}", layout.name));
            f.return_();
            f.end();
        }
    }
