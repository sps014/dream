...
impl Emitter<'_> {
    ...
    ///
    /// See [`Self::emit_call_args`]: arguments are already materialized MIR operands, so evaluating
    /// them cannot allocate. The callee roots its own params after the `call_indirect`.
