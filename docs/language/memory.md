...
- Mark sink parameters `take` (see [Functions](functions.md#ownership-transfer-take-and-explicit-borrow-borrow)) when the callee absorbs the value — e.g. `List.push(take value: T)` — so the store skips a redundant retain.
