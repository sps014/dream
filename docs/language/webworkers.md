...
**Hard abort:** `w.terminate()` (and `del`) kills the worker thread. Browser uses `Worker.terminate()`; native uses wasmtime epoch interruption so a busy body aborts. Pending `join()` settles (does not hang). Hard abort does **not** run Dream `finally` / orderly ARC unwind — prefer the token when `@shared` state must stay consistent.
