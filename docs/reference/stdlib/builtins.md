...
Example: `Math.sqrt(9.0)` → `3.0`. A negative input is NaN.

`Math.abs`, `floor`, `ceil`, `round`, `sqrt`, `pow`, `sin` / `cos` / `tan`, `asin` / `acos` / `atan` / `atan2`, `log` / `log10` / `exp` / `hypot`, `min` / `max` / `clamp`, and `Math.PI` / `Math.E`. Angles are radians.
