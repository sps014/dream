...
`CancellationSource` has `cancel()`, `is_cancelled()`, and `token()`. Pass the token to APIs such as [`HttpClient.with_cancellation`](http.md). `CancellationToken.is_cancelled()` is the read-only side.
